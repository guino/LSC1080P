...
filesize=`/mnt/busybox stat -c"%s" "$file"`
test "$filesize" -lt "$tail_len" && exit 1

# Check that tail is correct
/mnt/busybox dd if="$file" skip=$((filesize - tail_len)) bs=1 count=1000 >"$file.tail" 2>/dev/null
printf "\r\n%s--\r\n" "$delim_line" >"$file.tail.expected"
if ! /mnt/busybox diff -q "$file.tail" "$file.tail.expected" >/dev/null; then
...
# Truncate the file
/mnt/busybox dd of="$file" seek=$((filesize - tail_len)) bs=1 count=0 >/dev/null 2>/dev/null

# Move file
rm -f /mnt/$filename
mv $file /mnt/$filename
